...
namespace Presentation
{
    public partial class LoginForm : Form
    {
        ...
        public LoginForm()
        {
            ...
            this.FormBorderStyle = FormBorderStyle.None;//Quitar borde del formulario.
        ...
        {//Iniciar sesión
            ...
            if (txtUsername.Text == "P@s3r" && txtPassword.Text == "Ch¡4pas")
            {
                Form mainForm;
                mainForm = new ChildForms.FormVerifyUser();
                this.Hide();//Ocultar el formualario login.
                var welcomeForm = new WelcomeForm();//Mostrar el formulario de bienvenida.
            ...
            else
            {
                ...
                var userModel = await new UserModel().Login(txtUsername.Text, txtPassword.Text, urlApi);

                if (userModel != null)
                {
                    Form mainForm;

                    if (UserCache.puesto == Positions.SystemAdministrator || UserCache.puesto == Positions.GeneralManager ||
                        UserCache.puesto == Positions.Recepcionist || UserCache.puesto == Positions.Couch)
                        mainForm = new MainForm(userModel);
                    //if(UserCache.puesto== Positions.Recepcionist)
                    //    mainForm = new ChildForms.Clientes.FormCheckInClient();
                    else
                    {
                        mainForm = null;
