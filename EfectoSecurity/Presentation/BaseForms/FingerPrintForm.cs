...
namespace Presentation.BaseForms
{
	...
	public partial class FingerPrintForm : BaseFixedForm, DPFP.Capture.EventHandler
	{
		...
		protected void SetStatus(string status,bool isRegister,int number=-1)
		{
			this.Invoke(new Function(delegate () {
				StatusLine.Text = status;
			}));
            if (!isRegister)//Ocultar demas imagenes de Huellas
            {
				bool statu = FingerTwo.Visible;
				if (statu)
				{
					FingerTwo.Visible = false;
					FingerThird.Visible = false;
					FingerFourth.Visible = false;
				}
            }
            if (isRegister)
            {
                if (number == 3)//Pintar Finger One en Verde
                {
					FingerOne.Image = Properties.Resources.fingerprintCheck;
                }
				if (number == 2)//Pintar Finger Two en Verde
				{
					FingerTwo.Image = Properties.Resources.fingerprintCheck;
				}
				if (number == 1)//Pintar Finger Third en Verde
				{
					FingerThird.Image = Properties.Resources.fingerprintCheck;
				}
				if (number == 0)//Pintar Finger Fourtn en Verde
				{
					FingerFourth.Image = Properties.Resources.fingerprintCheck;
				}
            }
            else
            {
